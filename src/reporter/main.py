...
from reporter.utils.pdf_loader import load_pdfs_from_directory
...
output_path = os.path.join(
    base_path, f"output/reports/{timestamp}_{config.OUTPUT_PATH}"
)
...
def process_earnings_report() -> None:
    ...
    if check_redis_vectorstore_exists():
        ...
        pdf_chunks = load_pdfs_from_directory(pdf_path)
        if not pdf_chunks:
            logger.error("Error loading PDFs")
            sys.exit(1)
