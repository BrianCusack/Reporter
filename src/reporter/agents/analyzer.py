...
def analysis_agent( state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Initialize LLM
        llm = ChatAnthropic(
            model=config.CLAUDE_MODEL, temperature=0, api_key=config.ANTHROPIC_API_KEY
        )
        ...
        for query in questions:
            ...
            response = llm.invoke(
                f"""Based on the following excerpts from Tesla's updates, answer this question: 
        ...
        return_state.update({"analysis": analysis_results})

    except Exception as e:
        logger.error(f"Error during analysis: {str(e)}")
        return_state.update({"error": f"Error during analysis: {str(e)}"})
